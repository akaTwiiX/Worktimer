  ...
  const events: EventInput[] = docs.map((docData: any) => {
    ...
    if (isStartInMonth) {
      ...
      if (title.includes('<small>Kasse:</small>')) {
        ...
        const numParts = numbers.split('-');
        if (numParts.length === 2) {
          const numStart = Number.parseInt(numParts[0].trim(), 10);
          const numEnd = Number.parseInt(numParts[1].trim(), 10);

          if (!Number.isNaN(numStart) && !Number.isNaN(numEnd)) {
            const daySum = numEnd - numStart;
